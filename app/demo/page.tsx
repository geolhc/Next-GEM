...
function getAiGuidance(name: string, language: Language) {
  const guidance = {
    ...
    TC: [
      ...
      { eyebrow: "首個里程碑完成", message: "你的示範賬戶號碼已準備好，我可立即協助啟用最需要的服務。", prompts: [["現在可以使用甚麼？", "商務網上理財會先啟用，其他服務會按需要及檢查進度解鎖。"], ["賬戶號碼安全嗎？", "是。這是已遮蔽的虛構號碼，示範不會收集個人資料。"]] },
    ...
    SC: [
      ...
      { eyebrow: "首个里程碑完成", message: "你的演示账户号码已准备好，我可立即协助启用最需要的服务。", prompts: [["现在可以使用什么？", "商务网上理财会先启用，其他服务会按需要及检查进度解锁。"], ["账户号码安全吗？", "是。这是已遮蔽的虚构号码，演示不会收集个人资料。"]] },
...
function GemMark() { return <span className="gemMark" aria-hidden="true"><i /><i /><i /></span>; }
          ...
          <div className="demoWorkspace">
            <div className="demoContent">
              ...
              {step === 6 && <div className="demoPanel dashboardPanel">
                ...
                <section className="aiRecommendations" aria-labelledby="ai-recommendations-title">
                  ...
                  <div className="cardRecommendationGrid">
                    <article className="productRecommendation recommended">
                      <div className="productCardVisual"><img src={`${siteBasePath}/products/business-debit-mastercard.png`} alt="Business Debit Mastercard product card" /></div>
                    ...
                    <article className="productRecommendation">
                      <div className="productCardVisual dark"><img src={`${siteBasePath}/products/business-mastercard.png`} alt="Business Mastercard product card" /></div>
                ...
                <div className="demoFinish"><span>♡</span><p><b>{t("One relationship. No restart.", "一段關係，毋須重來。", "一段关系，无需重来。")}</b>{t("Your profile, progress and permissions move with you—across business and personal needs.", "你的資料、進度及權限會一直跟隨你，連接企業及個人需要。", "你的资料、进度及权限会一直跟随你，连接企业及个人需求。")}</p><a href={`${siteBasePath}/`}>{t("Return to pitch →", "返回簡介 →", "返回简介 →")}</a></div>
              </div>}
            </div>

            <aside className={`aiCompanion ${aiExpanded ? "open" : ""}`} aria-label="Next GEM AI guide"><button className="aiCompanionHeader" onClick={() => setAiExpanded((value) => !value)} aria-expanded={aiExpanded}><span><VisualIcon name="sparkle" /></span><p><b>Next GEM AI</b><small>{t("Responsible guide · Online", "負責任引導 · 在線", "负责任引导 · 在线")}</small></p><em>{aiExpanded ? "−" : "+"}</em></button><div className="aiCompanionBody"><p className="aiStageLabel">{guide.eyebrow}</p><div className="aiBubble"><i>AI</i><p>{guide.message}</p></div>{aiReply && <div className="aiBubble followup"><i>AI</i><p>{aiReply}</p></div>}<div className="aiPrompts">{guide.prompts.map(([label, answer]) => <button key={label} onClick={() => setAiReply(answer)}>{label} <span>→</span></button>)}</div><div className="aiGuardrail"><VisualIcon name="privacy" /><span><b>{t("You decide. AI assists.", "由你決定，AI 協助。", "由你决定，AI 协助。")}</b>{t("Permissions remain visible and reversible.", "所有權限保持清晰並可隨時撤回。", "所有权限保持清晰并可随时撤回。")}</span></div></div></aside>
          </div>
        </section>
      </div>
    </main>
